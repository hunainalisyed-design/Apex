...
- **Asset URLs are content-addressed** (`25-asset-versioning-cdn.md`): every GLB/GLTF/image the catalog points at is named `<name>.<8-hex sha256>.<ext>` and never overwritten in place — publish a new file with `npm run assets:version` (backend) and store its new URL. 3D views load a vehicle's `showroomModelUrl`/`heroModelUrl` from the API; don't hardcode model paths in the frontend.
- **UI text and numbers go through i18n** (`26-i18n-api-docs.md`): new or touched user-facing strings live in `frontend/messages/en-US.json` and are read with `useTranslations` (client/sync server components), `getTranslations` (async server components), or `translate` from `src/i18n/translator.ts` (outside React). Displayed numbers use `lib/format/` (`formatPriceCents`, `formatNumber`, `formatSavedDate`), never raw `toFixed`/template strings.
- **API docs are generated, not hand-written** (`26-i18n-api-docs.md`): a new backend route must get an entry in `backend/src/openapi/routes.ts` (or `UNDOCUMENTED_ROUTES` with a reason), and a DTO change needs `npm run openapi:generate` — `openapi.int.test.ts` fails otherwise.
