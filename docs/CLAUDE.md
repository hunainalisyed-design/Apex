...
This no longer blocks *implementation*, though. `04-landing-page.md` through `08-accessories-packages.md` have all proceeded on a procedural placeholder rig (primitive Three.js geometry, not a GLB) instead of waiting: Spec 4's hero is a simple decorative shape; Spec 5's showroom rig (`frontend/src/components/showroom/PlaceholderShowroomRig.tsx`) is a properly structured stand-in — named body/wheel/door/brake-caliper/headlight/brakelight meshes, real door-hinge animation, real emissive lighting; Spec 6 extended it with glass, spoiler, front/rear accessory, body-package, and carbon-trim meshes, all driven by real per-option `applyMode` (material/variant/visibility) dispatch; Spec 7 went further still — split the cabin into an opaque lower structure plus the glass greenhouse above it, and added five real interior surfaces (seat, dashboard, steering wheel, door-panel trim, floor) with grade+color material composition (`composeInteriorMaterial`), re-aiming the Interior/Cockpit camera presets to actually frame them. So paint/wheels/brake calipers/window tint/spoiler/accessories/body package/carbon trim/all seven interior categories genuinely change the scene now. Two real narrowings: wheel "style" is a color/finish variation on the same four wheel meshes, not distinct geometry per style; and interior "grade" is expressed through material roughness/metalness, not a texture/grain map (no texture assets exist). Both revisit once a real GLB exists.

Spec 8 confirmed the rig approach has a natural stopping point rather than stretching indefinitely: it added only three more meshes — a roof panel and mirror caps (`MATERIAL_SWAP`, paint-colored by default, carbon-finished when active) and an exhaust tip (`MESH_VISIBILITY`, hidden by default) — and deliberately did **not** fabricate visuals for the three accessories/packages with no obvious single mesh (Premium Lighting Package, Performance Package, Special Interior Package). Per its own AC-6, those three price and appear in the summary correctly while logging a dev-only console warning instead. This establishes the pattern going forward: extend the placeholder rig only where an option maps cleanly to one mesh, and use the warn-don't-fabricate path everywhere else, rather than inventing placeholder geometry for effects that don't have an obvious 3D representation. The real GLB decision (Spec 2 Risk #1) remains open and should be resolved before any spec that needs texture/grain detail or genuinely distinct geometry per option (e.g. real wheel-style variants).

**Target aesthetic once a real GLB is sourced:** the user wants the eventual visualization to look like a Rolls-Royce online configurator — photorealistic and luxury-grade, not the current placeholder's flat primitive shading. That means: a studio HDRI environment with three-point lighting and a reflective floor/contact shadow; glossy clearcoat automotive paint with metallic flake, mirror-chrome trim, tinted reflective glass, and matte-tire/alloy-wheel materials; a low 3/4 hero camera angle with shallow depth of field and ambient occlusion; and a minimal circular color-swatch picker with a color-name label and a smooth fade transition on change (see Spec 2 Risk #1 for the full brief). This is the acceptance bar for whichever future spec finally replaces the placeholder rig with a real asset — it is not something to attempt on the current primitive geometry.

**Update: the body/wheels/spoiler are no longer procedural.** The showroom now loads real geometry for those three parts from Kenney's "Car Kit" (`sedan-sports.glb`, CC0, `frontend/public/models/shared/`, credited in that folder's `CREDITS.md`) via `frontend/src/components/showroom/CarModel.tsx`'s `useCarModelGeometry` hook — every other mesh (glass, brake calipers, all interior surfaces, doors, mirrors, roof, exhaust tips, headlights/brakelights, front/rear accessory, body-package skirts, carbon trim) is still the same procedural geometry as before, just repositioned to fit the real body's actual (larger, differently-proportioned) shape. `VehicleDetailDto.showroomModelUrl` is now genuinely read (`ConfigureShowroom.tsx` passes it into `ShowroomScene`/`PlaceholderShowroomRig`); both seeded vehicles currently point at the same shared asset since no per-vehicle differentiation exists yet. The landing-page hero (`PlaceholderVehicleMesh.tsx`) is untouched and still fully procedural. This closes the "separately-addressable wheel meshes" half of Spec 2 Risk #1 in practice, but the Kenney kit's flat game-asset materials are nowhere near the "Target aesthetic" paragraph above — that photorealistic bar is still fully open and is a distinct, larger future effort (a proper PBR asset + HDRI), not something this change attempted.
