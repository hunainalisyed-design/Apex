...
This no longer blocks *implementation*, though. `04-landing-page.md`, `05-3d-showroom-core.md`, and `06-exterior-customization.md` all proceeded on a procedural placeholder rig (primitive Three.js geometry, not a GLB) instead of waiting: Spec 4's hero is a simple decorative shape; Spec 5's showroom rig (`frontend/src/components/showroom/PlaceholderShowroomRig.tsx`) is a properly structured stand-in — named body/wheel/door/brake-caliper/headlight/brakelight meshes, real door-hinge animation, real emissive lighting; Spec 6 extended it further with glass, spoiler, front/rear accessory, body-package, and carbon-trim meshes, all driven by real per-option `applyMode` (material/variant/visibility) dispatch, so paint/wheels/brake calipers/window tint/spoiler/accessories/body package/carbon trim all genuinely change the scene. The one real narrowing: wheel "style" (Standard/Sport/Performance/Carbon) is a color/finish variation on the same four wheel meshes, not distinct geometry per style — revisit once a real GLB exists. Spec 7 (interior customization) is the next one to determine whether this rig approach keeps stretching or whether the real GLB decision needs resolving first — evaluate that at the start of Spec 7 rather than assuming either way.
