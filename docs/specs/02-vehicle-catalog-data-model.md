...
|---|---|---|---|
| 1 | No production 3D asset exists yet. Need one free, portfolio-safe (CC0/permissive license) GLB car model with separably-addressable wheel meshes and a body material slot, used as the placeholder for both seeded vehicles until real assets are commissioned. | Product owner | Partially resolved — the showroom's `body`, 4 wheels, and `spoiler` now use real geometry from Kenney's "Car Kit" (`sedan-sports.glb`, CC0, see `frontend/public/models/shared/CREDITS.md`), replacing the equivalent procedural primitives in `frontend/src/components/showroom/PlaceholderShowroomRig.tsx`; every other mesh (glass, brake calipers, interior surfaces, doors, mirrors, roof, exhaust tips, headlights/brakelights, front/rear accessory, body-package skirts, carbon trim) is still procedural, repositioned to fit the real body. `VehicleDetailDto.showroomModelUrl` is now actually read (both seeded vehicles point at the same shared asset — no per-vehicle differentiation yet); `heroModelUrl`/the landing hero remain unwired. Still open beyond this: true photorealism — a studio HDRI environment + three-point lighting + reflective floor with contact shadow; glossy clearcoat metallic-flake body paint, mirror-chrome trim, tinted reflective glass, matte tires with brushed/polished alloy wheels; a low 3/4 hero camera angle with shallow depth of field and ambient occlusion in panel gaps; and a minimal circular color-swatch picker with a color-name label and a smooth fade transition on change (the Rolls-Royce-configurator-style target aesthetic). None of that is achievable with the Kenney kit's flat game-asset materials — it needs a proper PBR asset and HDRI, so it remains the acceptance bar for a future spec, not something this change attempted. |
