...
- **AC-4's "failed" confirmation-page state was originally unreachable** — the webhook handler only ever listened for `checkout.session.completed`, so an abandoned or declined checkout left its `Reservation` stuck `PENDING` forever, indistinguishable from "the webhook just hasn't landed yet" on the polling confirmation page. Fixed by also handling Stripe's `checkout.session.expired` and `checkout.session.async_payment_failed` webhook events, both mapped to `status: FAILED` via the same idempotent, self-healing `applyTerminalStatus` helper `handleCheckoutSessionCompleted` already used (`backend/src/services/reservations/webhook.ts`). That helper also gates each update on the row's *current* status (`PENDING` → terminal only), so a late or out-of-order `expired` delivery can never clobber a `Reservation` a `completed` event already marked `PAID`.
