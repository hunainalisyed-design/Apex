...
| 4 | Transactional email sending (password reset, AC-6/AC-7) requires a real provider (e.g. Resend, SendGrid, Postmark) with its own API key and sender-domain verification. | Implementer | Open — pick a provider at implementation time; recommend Resend for simplicity given the Next.js-adjacent stack, but this is not architecturally significant enough to block spec approval on. |
